...
export function DeleteWordDialog({
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => wordService.delete(id),
    onSuccess: () => {
  ...
  const handleConfirmDelete = () => {
    if (!word?._id) return;
    setErrorMessage(null);
    deleteMutation.mutate(word._id);
  };
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="space-y-1.5 flex-1 pr-2">
            <DialogTitle className="text-base font-bold text-slate-900">
              {t('delete_word_title', { defaultValue: 'Confirm Delete Word' })}
            </DialogTitle>
            <DialogDescription className="text-xs text-slate-500 leading-relaxed">
              {t('delete_word_desc', {
                word: word?.word || '',
                defaultValue: `Are you sure you want to delete the word "${word?.word}"? This action will move the word to the deleted list.`,
              })}
            </DialogDescription>
          </div>
        </div>
